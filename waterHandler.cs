...
namespace SeeloewenCraft
{
    public class WaterHandler
    {
                    ...
                    if (block.name == "Water")
                    {
                        ...
                        if (block.hasWaterSource)
                        {
                            ...
                            if (sourceChunk != null)
                            {
                                ...
                                if (sourceBlock != null && !sourceBlock.isWaterSource)
                                {
        ...
        private void ExpandWater(Chunk chunk, Block block, List<Block> newBlocks)
        {
            ...
            if (blockBelow != null && (!blockBelow.isSolid && (blockBelow.name != "Water" || blockBelow.waterLevel < 6)) || (blockBelow.isBackground && blockBelow.foregroundBlock == null))
            {
            ...
            else if (blockBelow != null && (blockBelow.isSolid && blockBelow.name != "Water") || (blockBelow.isBackground && blockBelow.foregroundBlock != null && blockBelow.foregroundBlock.name != "Water"))
            {
                //Only expand if the water hasn't reached the end of the stream
                if (block.waterLevel != 1)
                {
        ...
        private void UpdateBlocks(List<Block> newBlocks)
        {
            ...
            {
                if (block.yPos > 0 && block.yPos < 76)
                {
                    //Check if a new water blocks needs to be placed or a block needs to be removed
                    if (block.name == "Water")
                    {
                        if (block.chunk.GetBlock(block.xPos, block.yPos).isBackground)
                        {
                        ...
                        else
                        {
                            ...
                            block.chunk.SetBlock(block, block.xPos, block.yPos);
                        }
                    }
                    else
                    {
